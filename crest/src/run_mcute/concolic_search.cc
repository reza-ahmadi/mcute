...
Search::Search(const string& program, int max_iterations)
  ...
  { // Read in the set of branches.
	...
	//reza start
//	cout<< "^^^^^^^^program " << program.c_str() << ", branch: " << "branches_"+program << endl;
	//reza end

	ifstream in("branches_"+program);
    assert(in);
    function_id_t fid;
    int numBranches;
...
void Search::LaunchProgram(const vector<value_t>& inputs) {
  //reza start
  std::cout<<"%%%%%%%%%running the program"<<program_.c_str();
  //reza end
...
//START-REZA: Save the given inputs.
  char fname[32];
